...
spread %>% ggplot + aes(date, count, color=location) + geom_point()  + 
                                        scale_y_log10(limit=c(1e3,1e6))  + labs(caption=capt) + 
                                        xlab("Days since Jan 22, 2020") + ylab("Mortality") + ggtitle("COVID-19 deaths by continent") 
                                        

 filename_base = "covid-global-deaths-continents"
